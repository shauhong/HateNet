...
class ViltEmbeddings(nn.Module):
    ...
    def visual_embed(self, pixel_values, pixel_mask, max_image_length=200):
        ...
        x_mask = nn.functional.interpolate(
            x_mask, size=(x.shape[2], x.shape[3])).long()
        x_h = x_mask[:, 0].sum(dim=1)[:, 0]
        x_w = x_mask[:, 0].sum(dim=2)[:, 0]

        batch_size, num_channels, height, width = x.shape
        patch_dim = self.config.image_size // self.config.patch_size
        spatial_pos = self.position_embeddings[:, 1:, :].transpose(
            1, 2).view(1, num_channels, patch_dim, patch_dim)
        pos_embed = torch.cat(
            [
                nn.functional.pad(
        ...
        patch_index = patch_index.expand(
            x_mask.shape[0], x_mask.shape[1], -1, -1, -1)
        ...
        if max_image_length < 0 or max_image_length is None or not isinstance(max_image_length, int):
            ...
            effective_resolution = x_h * x_w
            max_image_length = min(
                effective_resolution.max(), max_image_length)
        ...
        patch_index = patch_index[select[:, 0],
                                  select[:, 1]].view(batch_size, -1, 2)
...
class ViltEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([ViltLayer(config)
    ...
    def forward(
        ...
        if attention_mask is None:
            attention_mask = torch.ones(
                ((text_batch_size, seq_length)), device=device)
...
class ViltForMaskedLM(ViltPreTrainedModel):
    ...
    def forward(
        ...
        text_features, _ = (
            sequence_output[:, :text_seq_len], sequence_output[:, text_seq_len:])
...
class ViltForImagesAndTextClassification(ViltPreTrainedModel):
    def __init__(self, config):
        ...
        self.classifier = nn.Sequential(
            nn.Linear(config.hidden_size * num_images,
                      config.hidden_size * num_images),
        ...
        for i in range(num_images):
            ...
            outputs = self.vilt(
                ...
                pixel_mask=pixel_mask[:, i, :,
                                      :] if pixel_mask is not None else None,
                head_mask=head_mask,
                inputs_embeds=inputs_embeds,
                image_embeds=image_embeds[:, i, :,
